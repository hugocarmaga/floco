            ...
            if len(aln_nodes) == 1:
                if nodes.get(aln_nodes[0][0]) != None:
                    if aln_nodes[0][1]:    # Positive strand
                        ...
                        if nodes[aln_nodes[0][0]].bins != None: update_bins(nodes[aln_nodes[0][0]], start_pos, end_pos)
    ...
    for node in nodes_to_bin:
        for (bin_size, cov_bins) in nodes[node].bins:
            if bin_size == sel_size:
                ...
                if cov_bins[cov_bins >= sel_size].size and cov_bins.size >= 10:
